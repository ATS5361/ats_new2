...
#            cls._instance.connect_to_database()
    ...
    def connect_add_user_db(self, psw):
        self.conn1 = sql.connect('home/tai-orin/Desktop/ats_new2/database_files/ADMIN.db')
        admin_cursor = self.conn1.cursor()
        admin_cursor.execute("SELECT USERNAME, LASTNAME, DEPARTMENT, PASSWORD FROM ADMIN WHERE PASSWORD =:password", {"password":psw.upper()})
        self.admin_data = admin_cursor.fetchall()
        return self.admin_data

    def connect_login_user_db(self, psw):
        self.conn2 = sql.connect('home/tai-orin/Desktop/ats_new2/database_files/LOGIN.db')
        login_cursor = self.conn2.cursor()
        login_cursor.execute("SELECT USERNAME, LASTNAME, DEPARTMENT, PASSWORD FROM LOGIN WHERE PASSWORD =:password", {"password":psw.upper()})
